/*********************************************************************
** Program name: otp_enc_d (HW4)
** Author: Charles Ledbetter
** Date: 3/14/2018
** Description: The server side of the encryption program. This
program acts as a daemon that listens on a particular port for data
from the server side encryption program. It will only return encrypted
data to the client side partner of this program. This program takes
the message and encryption key and forks a child to encrypt the
...
int main(int argc, char *argv[]){
	...
	//used to check that opt_enc (not opt_dec) is the contacting client
	char cArg[10];
	char* encName = "otp_enc";

	//used to parse data from recBuff encode it and send it back
	char mesBuff[80000];
	char keyBuff[80000];
	...
	while(1){
		...
		for(i = 0; i < 5; i++){
			...
			if(encPids[i] != -5){
				...
				if(waitpid(encPids[i], &pidStatus, WNOHANG)){
					encPids[i] = -5;
				}
			}
		}
		...
		// find a encPid not in use and fork
		for(i = 0; i < 5; i++){
			if(encPids[i] == -5){
				encPids[i] = fork();

				//if the fork failed throw an error
        if(encPids[i] < 0 || establishedConnectionFD < 0){
					fprintf(stderr,"ERROR on accept\n");
					exit(1);
        }


				//child encrypts message
				else if(encPids[i] == 0){
					...
					//if cArg is otp_enc tell the client
					if(strcmp(cArg, encName) == 0){
						for(i = 0; i < strlen(mesBuff); i++){
							...
							//encode the message
								...
								encoded = (((mesBuff[i] - 65) + (keyBuff[i] - 65)) % 27) + 65;
								...
								mesBuff[i] = (char)encoded;
							}

							//for debugging incorrect encryption
							else{
								mesBuff[i] = '#';
							}
						}
					...
					//if cArg is not otp_enc tell the client
					else{
						mesBuff[0] = '@';
					}
					...
					//********End send encrypted data back to client********//
