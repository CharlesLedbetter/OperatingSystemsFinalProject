/*********************************************************************
** Program name: otp_dec (HW4)
** Author: Charles Ledbetter
** Date: 3/14/2018
** Description: The client side of the one time pad decryption Program
This program takes a encrypted message and a key and sends it over a
port to the server side decryption program. The server then sends back
decrypted data. The data is then checked to make sure it is the same
length as the original message. If it is the decrypted message is
printed to the screen. If it is not, the process starts again sending
the message until a correctly decrypted message returns.
*********************************************************************/
#define _GNU_SOURCE
...
int main(int argc, char *argv[]){
	...
	do {
		...
		for(i = 0; i < sizeof(mesBuff); i++){
			...
			if(mesBuff[i] > 90 || (mesBuff[i] < 65 && mesBuff[i] != 32)){
				fprintf(stderr,"Invalid data for decryption message\n");
				exit(1);
			}
		}
		...
		for(i = 0; i < sizeof(keyBuff); i++){
			...
			if(keyBuff[i] > 90 || (keyBuff[i] < 65 && keyBuff[i] != 32)){
				fprintf(stderr,"Invalid data for decryption key\n");
				exit(1);
			}
		}
	...
	//********Contact server and send message********//
		...
		do {
			...
			//if the opt_dec_d server is not the server contacted throw an error
			if(retBuff[0] == '@'){
				fprintf(stderr,"ERROR opt_dec must not connect to opt_enc_d\n");
				exit(2);
			}

			//if the opt_dec_d server did not receive the message exit
